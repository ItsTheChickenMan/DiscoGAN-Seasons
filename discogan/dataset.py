...
summer2winter_yosemite_path = os.path.join(dataset_path, "summer2winter_yosemite")

def shuffle_data(da, db):
    rng = np.random.default_rng()
    
    shuffled_da = np.array(da)
    shuffled_db = np.array(db)
    
    rng.shuffle( shuffled_da )
    rng.shuffle( shuffled_db )
...
def get_summer2winter_yosemite_files(test=False):
    dirs = ["testA", "testB"] if test else ["trainA", "trainB"]
    dirs = [ os.path.join(summer2winter_yosemite_path, d) for d in dirs ]
